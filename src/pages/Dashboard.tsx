...
import ProposalRecommendations from '../components/ProposalRecommendations';
...
const Dashboard = () => {
  ...
  const overdueProposals = proposals.filter(p => isOverdue(p.dueDate) && p.status !== 'submitted').length;
  ...
  const completedProposals = proposals.filter(p => p.status === 'submitted').length;
